 ...
 */
class M_ComparisonChart extends IChart {
  element_id = "match_ComparisonChart";
  ...
  prepareData(commitData, propertyName) {
    ...
    for (var i = 0; i < chartColumns.length; i++) {
      try {
        //console.log("Item: " + i + " -- " + leftItem);

        if (Array.isArray(commitData[chartColumns[i] + "_l"])) {
          leftItem = commitData[chartColumns[i] + "_l"].length;
          rightItem = commitData[chartColumns[i] + "_r"].length;
        } else {
          leftItem = commitData[chartColumns[i] + "_l"];
          rightItem = commitData[chartColumns[i] + "_r"];
        }
